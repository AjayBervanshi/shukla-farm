...
function DashboardPage() {
  ...
  return (
    <AppLayout>
      ...
      {/* Today's slots */}
      <div className="mb-6">
        <h2 className="font-display text-xl mb-4 gold-text">Today's Slots</h2>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className={`px-2.5 py-1 text-[10px] uppercase tracking-wider font-bold rounded-full ${booking ? "bg-danger/20 text-danger" : "bg-success/20 text-success"}`}>
                ...
                <div className="mt-4 pt-4 border-t border-border/40 text-sm">
                  {booking ? (
                    ...
                    >
                      {booking.client_name} <span className="text-muted-foreground ml-1 font-normal">• {booking.event_type}</span>
                    </Link>
                  ) : (
                    <span className="text-success/80 italic text-sm">Ready to book</span>
                  )}
                </div>
              </div>
            );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="luxury-card p-6 lg:col-span-2">
          <h2 className="font-display text-xl mb-6 gold-text">Monthly Revenue</h2>
          {dash.isLoading ? (
            <Skeleton className="h-64 w-full rounded-xl" />
          ) : (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={dash.data?.chart ?? []}>
                <CartesianGrid stroke="var(--color-border)" opacity={0.3} vertical={false} strokeDasharray="4 4" />
                <XAxis
                  dataKey="month"
                ...
                />
                <Bar dataKey="revenue" fill="var(--color-gold)" radius={[6, 6, 0, 0]} maxBarSize={40} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
  ...
  return (
    <div className="luxury-card luxury-card-hover p-5">
      <div className="flex items-center justify-between">
        ...
        <span className={`p-2 rounded-lg ${gold ? "bg-gold/10 text-gold" : warning ? "bg-warning/10 text-warning" : "bg-primary/10 text-primary"}`}>
          {icon}
        </span>
      </div>
      <div
        className={`mt-4 font-mono-num text-3xl font-bold tracking-tight ${
          gold ? "text-gold" : warning ? "text-warning" : "text-foreground"
        }`}
      >
        {value}
      </div>
